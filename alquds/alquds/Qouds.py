...
@frappe.whitelist()
def get_context(docName,docType):
	...
	make_access_log(
		doctype=docType, document=docName, file_type="PDF", method="Print"
	)
...
def get_rendered_template(
	...
	print(f"doc {doc} \n\n")
	print(f"meta {frappe.get_meta(doc.doctype)}\n\n")
	print(f"layout {make_layout(doc, meta, format_data)}\n\n")
	print(f"trigger_print {trigger_print}\n\n")
...
def get_html_and_style(
	...
	print_format = frappe.get_doc("Print Format", doct.print_format)
...
# 	if isinstance(doc, str) and isinstance(name, str):
# 		doct = frappe.get_doc(doc, name)
# 		print(f"doct {doct.doctype}")
# 	# if isinstance(doc, str):
# 	# 	doct = frappe.get_doc(json.loads(doc))

# 	# print_format = get_print_format_doc(print_format, meta=meta or frappe.get_meta(doct.doctype))
# 	print_format = frappe.get_doc("Print Format", doct.print_format)
# 	print(f"PRINT FORMAAT {doct.print_format}")
# 	# print(frappe.get_meta(doct))
...
# 	return {
# 		"raw_commands": get_rendered_template(doct, name=name, print_format=print_format, meta=meta)
# 	}
